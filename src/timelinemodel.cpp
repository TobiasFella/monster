...
TimelineModel::TimelineModel(QObject *parent)
  ...
  connect(Dispatcher::instance(), &Dispatcher::timelineUpdate, this,
          [this](const auto &matrixId, const auto &roomId, const auto op,
...
Connection *TimelineModel::connection() const { return d->connection; }
...
QString TimelineModel::roomId() const { return d->m_roomId; }
...
QVariant TimelineModel::data(const QModelIndex &index, int role) const {
  ...
  if (role == IdRole) {
    return stringFromRust((*d->timeline)->timeline_item(row)->id())
        .toHtmlEscaped();
  }
  if (role == BodyRole) {
...
void TimelineModel::timelineUpdate(std::uint8_t op, std::size_t from,
                                   std::size_t to) {
...
bool TimelineModel::canFetchMore(const QModelIndex &) const { return true; }
...
bool ReversedTimelineModel::lessThan(const QModelIndex &sourceLeft,
