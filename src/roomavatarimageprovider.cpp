...
QQuickImageResponse *
RoomAvatarImageProvider::requestImageResponse(const QString &id,
                                              const QSize &requestedSize) {
  return new RoomAvatarImageResponse(id, requestedSize, m_connection.get());
}

RoomAvatarImageResponse::RoomAvatarImageResponse(const QString &id,
                                                 const QSize &,
                                                 Connection *connection) {
  connection->connection()->room_avatar(stringToRust(id));
  connect(Dispatcher::instance(), &Dispatcher::avatarLoaded, this,
          [id, this](const auto &roomId, const QByteArray &data) {
            if (id != roomId) {
              return;
            }
...
Connection *RoomAvatarImageProvider::connection() const { return m_connection; }
