...
RoomsModel::RoomsModel(QObject *parent)
  ...
  connect(this, &RoomsModel::connectionChanged, this,
          [this]() { d->rooms = d->connection->connection()->slide(); });
  connect(Dispatcher::instance(), &Dispatcher::roomsUpdate, this,
          [this](const auto &matrixId, const auto op, const auto from,
                 const auto to) {
...
QVariant RoomsModel::data(const QModelIndex &index, int role) const {
  ...
  if (role == IdRole) {
    ...
    return stringFromRust((*d->rooms)->room(row)->display_name())
        .toHtmlEscaped();
  } else if (role == AvatarUrlRole) {
    return QStringLiteral("image://roomavatar/%1")
        .arg(stringFromRust((*d->rooms)->room(row)->id()));
  }
  return {};
}
...
void RoomsModel::roomsUpdate(std::uint8_t op, std::size_t from,
                             std::size_t to) {
...
Connection *RoomsModel::connection() const { return d->connection; }
